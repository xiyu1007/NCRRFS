function recorder = run(method, X, Y, params, numRepeat, numFold, varargin)
    ...
    if ~skipExist
        warning("Training will not skip existing parameters!\n");
    end
    ...
    for pidx = 1:numParams
        ...
        if useParfor
            parfor idx = 1:numTotal
                ...
                try
                    ...
                    stackInfo = ME.stack;
                    fprintf('Worker %d failed at task %d: %s\n', getCurrentTask().ID, idx, ME.message);
        ...
        for i = 1:numFeatures
            nf = FeatureSpace(i);
            track = track.addParameters([nf, param], resultsList(i,:));
        ...
        for i = 1:numRepeat
            for j = 1:numFold
                [~, localRe.FSID{i,j}] = FeatureSelection(Xtrain{(i-1)*numFold+j}, Wlist{(i-1)*numFold+j}, 10);
        ...
        if verbose
            fprintf('%-3d/%-3d | %-.2fs | param: %s \t acc=%-5.4f(±%-5.4f)\t bestAcc=%-5.4f(±%-5.4f)\n', ...
                pidx, numParams, elapsed, sprintf('%-5.3g ', param), acc, stdAcc, globalAcc, globalStd);
        end
    ...
    if skipped == numParams
        sprintf('所有参数均已存在于 pt 中，未执行任何计算！\n');
    else
        recorder = saveRecorderState(recorder,track,ins,seeds,method,taskName);
    end
end
...
function recorder = saveRecorderState(recorder,track,ins,methodSeeds,method,taskName)
    ...
    recorder.pt = track;
    recorder.save(['temp\',taskName,'_',method,'_re_temp.mat'], 0);
end
