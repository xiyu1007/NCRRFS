function results = Evaluate(X_train,X_test,Y_train,Y_test, varargin)
    ...
    for i = 1:numFeatures
        ...
        % opt_weight = getWeight(nfxtrain,Y_train,kernel);
...
function opt_weight = getWeight(nfxtrain,Y_train,kernel)
    if numel(nfxtrain) ~= 2
        opt_weight = ones(1, numel(nfxtrain)) ./ numel(nfxtrain);
    else
        options = '-s 0 -t 4 -v 5 -q';
        best_accuracy = -Inf;
        opt_weight = NaN;
        for w = 0.1:0.1:0.9
            weight = [w, 1-w];
            k_train = mk_kernel(nfxtrain, 'weights', weight, 'kernels', {kernel});
            % accuracy = mk_svm(k_train, Y_train, options);
            evalc('accuracy = mk_svm(k_train, Y_train, options)');
